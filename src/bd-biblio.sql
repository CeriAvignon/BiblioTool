...
--
-- Client :  127.0.0.1
-- Généré le :  Ven 27 Janvier 2017 à 02:56
...
CREATE TABLE IF NOT EXISTS `citations` (
  `id_art_citing` int(11) NOT NULL,
  `id_art_cont` int(11) DEFAULT NULL,
  PRIMARY KEY (`id_art_citing`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `citer` (
  `id_art` int(11) NOT NULL,
  `id_art_citing` int(11) NOT NULL,
  PRIMARY KEY (`id_art`,`id_art_citing`),
  KEY `FK_citer_id_art_citing` (`id_art_citing`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
--
ALTER TABLE `citer`
  ADD CONSTRAINT `FK_citer_id_art_citing` FOREIGN KEY (`id_art_citing`) REFERENCES `citations` (`id_art_citing`),
